...
from fastsim.solvers import RKCK54, ESDIRK32
...
class TestLinearizeAndRun(unittest.TestCase):
    ...
    def test_linearize_not_implemented(self):
        """fastsim does not (yet) implement the operating-point linearization
        pathsim provides. linearize()/delinearize() must raise
        NotImplementedError rather than silently no-op (which would give false
        confidence). The steady-state solve itself still works."""
        ...
        #steady-state solve still works and lands on the equilibrium
        Sim.steadystate(reset=True)
        self.assertAlmostEqual(DS.outputs[0], 1.0, 3)

        #linearization is a documented gap -> explicit error, not a no-op
